...
public class BagTest {
    @Test
    void shouldTakeRuleAndApply() {
        List<Rules> rules =  new ArrayList<>();
        rules.add(Rules.BAG_CAPACITY_OF_TWELVE);
        Bag bag = new Bag(rules);

        bag.addBall(Ball.GREEN);

        Bag emptyBag = new Bag(rules);
        assertNotEquals(emptyBag, bag);
    }

    @Test
    void shouldThrowWhenCapacityRuleFails() {
        List<Rules> rules =  new ArrayList<>();
        rules.add(Rules.BAG_CAPACITY_OF_TWO);
        Bag bag = new Bag(rules);
